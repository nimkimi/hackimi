...
**Design direction (2026-06-02): a "designer × developer" portfolio with a signature ~2s arrival animation, mobile-first.** Nima is a developer AND a designer (design-engineer hybrid), so the site should read like a strong designer's portfolio backed by developer substance — designer-grade typography/color/layout + real engineering credibility (three flagship case studies). A signature ~2-second arrival/intro animation creates a positive feeling on first load (once per session; instant under `prefers-reduced-motion`). Motion is expressive but PURPOSEFUL — nothing random/decorative. Slick color. A reduced-motion fallback is **mandatory** (Nima is an accessibility specialist). Concrete art direction is being finalized via research + prototype; see the spec's latest "Direction Change" section.

Abandoned explorations (all built + rejected, kept under `mockups/` for history only): refined-minimal multi-page (`variant-a`) & long-scroll (`variant-b`) — too understated; high-craft editorial (`v2`) — too quiet/generic; interactive 3D galaxy (`cosmos`) — felt random/gimmicky. The react-three-fiber/3D direction is dead.
